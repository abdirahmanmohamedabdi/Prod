...
export default function OperatingIncome() {
  ...
  useEffect(() => {
    const fetchOperatingIncome = async () => {
  ...
  const addIncome = async () => {
    if (newIncome.name && newIncome.amount && newIncome.date && newIncome.term && newIncome.payment_method) {
      ...
      const { data, error } = await supabase.from('operating_incomes').insert([{
        name: newIncome.name,
  ...
  return (
    <Sidebar>
      <div className="p-6 min-h-screen">
        <ToastContainer />
        <h1 className="text-2xl font-bold mb-4 font-font text-center">Operating Income</h1>
        <table className="min-w-full bg-white border border-gray-200 rounded-lg">
          ...
          <tbody>
            {incomes.map((income, index) => (
              <tr key={index}>
                <td className="px-4 font-font py-2 border-b">{income.name}</td>
        ...
        <div className="mt-6">
          <h2 className="text-xl font-font font-semibold mb-2">Add New Income</h2>
          <div className="flex flex-wrap sm:flex-nowrap gap-4">
            ...
            <button
              onClick={addIncome}
              className="bg-one text-white px-4 py-2 font-font rounded hover:bg-blue-700"
            >
              Add Income
            </button>
          </div>
